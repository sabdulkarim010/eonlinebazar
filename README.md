...
| [DATABASE_MIGRATION_AUDIT.md](DATABASE_MIGRATION_AUDIT.md) | MongoDB → PostgreSQL (Prisma + Neon) migration: 5-stage roadmap, model-by-model mapping, cascade and index strategy, and the stage-by-stage change log. **Stage 2 Step 3 dual-write complete (2026-09-14). Stage 3 backfill in progress — Steps 1–3 (2026-09-14) via `backend/scripts/backfill/`: catalog/ERP + CMS/settings + security/audit + User/HRM/Marketing-Support groups migrated to Neon (idempotent via `legacyId` / singleton `key=global`; in-memory FK maps for Product/User/Admin/Employee). All live reads still come from MongoDB only.** |

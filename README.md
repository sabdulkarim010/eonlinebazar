...
| [DATABASE_MIGRATION_AUDIT.md](DATABASE_MIGRATION_AUDIT.md) | MongoDB → PostgreSQL (Prisma + Neon) migration: 5-stage roadmap, model-by-model mapping, cascade and index strategy, and the stage-by-stage change log. **Stage 2 Step 3 dual-write complete (2026-09-14). Stage 3 backfill in progress — Steps 1–4 (2026-09-14) via `backend/scripts/backfill/`: catalog/ERP through Admin + Product + gap repair (CartItem/Review/WishlistItem FK closure). In-memory FK maps throughout; Admin bcrypt hashes preserved on backfill. Documented permanent gaps: 3 users missing `firstName`, 1 orphaned Attendance (`nurjahan`). Order group pending. All live reads still come from MongoDB only.** |

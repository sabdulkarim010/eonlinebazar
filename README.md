...
| **Advanced POS Dashboard** | Walk-in and phone orders (`orders-pos.html` + `orders-pos.js`): debounced barcode/SKU/product-name search (`GET /api/products?search=&limit=5`) with live dropdown, 20-product popular quick-grid with Refresh, and an auto-open receipt modal (print + `GET /api/orders/:id/invoice` PDF). |

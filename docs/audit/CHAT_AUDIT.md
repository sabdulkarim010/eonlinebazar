...
| `ecommerce-chat/socket/chat.socket.js` | Socket.IO `/customer` + `/admin` namespaces |
| `ecommerce-chat/models/ChatRoom.model.js` | Chat room state |
| `ecommerce-chat/models/ChatMessage.model.js` | Messages |
| `ecommerce-chat/models/Agent.model.js` | Chat agents |
| `ecommerce-chat/models/AIKnowledgeBase.model.js` | KB entries |
| `ecommerce-chat/models/CannedResponse.model.js` | Quick replies |
| `ecommerce-chat/models/ChatSettings.model.js` | Chat config |
| `ecommerce-chat/models/StoreUser.model.js` | Store-linked users |
| `ecommerce-chat/services/ai.service.js` | OpenAI + KB fallback bot |
| `ecommerce-chat/services/agentResolver.service.js` | Store admin → Agent SSO |
| `ecommerce-chat/services/storeProfile.service.js` | CRM enrichment from store API |
...
| `client/js/admin/modules/chat-admin.js` | Legacy chat admin JS |
| `mobile/src/screens/LiveSupportScreen.js` | Mobile live support |

**Counts (verified 2026-09-20):** 7 chat models, 37 JS files in `ecommerce-chat/`.
...
- [x] Chat CRM enrichment — `storeProfile.service.js`, `internalChatController.js`
- [x] Mobile live support — `LiveSupportScreen.js`, `useAriaChat.js`
- [x] Agent auto-provision from store admin — `agentResolver.service.js`
- [x] Legacy `view-chat` deprecation notice — redirect to `/chat-admin`
- [ ] Close/end session UX (SweetAlert2) — native `confirm()` still used
- [ ] Customer widget session teardown on close — re-bootstraps room after close
- [ ] Full Agent ↔ Admin SSO | Partial auto-provision only
...
- **Ports:** chat `:5001`; proxied via store gateway
- **Env:** `CHAT_SERVICE_URL`, `INTERNAL_API_KEY`, `EXPO_PUBLIC_CHAT_URL`, `VITE_API_URL`, `VITE_SOCKET_URL`
- **Separate DB:** Chat microservice uses its own MongoDB (not main store DB)
- **Related:** `docs/SETUP.md`, `SYSTEM_ENTERPRISE_AUDIT.md` (Live Chat section)
...
- Added File Inventory (37 ecommerce-chat files + integrations) and Feature Checklist
- Preserved detailed close/session findings from 2026-08-25 below
- Status: ⚠️ PARTIAL (close/teardown + WhatsApp/OpenAI external deps)

### Initial close/session audit — 2026-08-25

- Migrated from root `CHAT_AUDIT.md`; documented alert + teardown gaps

---

## Detailed Findings (2026-08-25)

**Original scope:** Customer widget, Staff/Admin dashboard, and backend socket teardown — audit only, no fix code in this pass.

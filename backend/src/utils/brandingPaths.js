...
const BRANDING_DIR = path.join(REPO_ROOT, 'client', 'uploads', 'branding');
...
function resolveBrandingDiskPath(publicUrl) {
    ...
    return path.join(REPO_ROOT, 'client', normalized.replace(/^\//, ''));
}
...
function deleteLocalBrandingAsset(publicUrl) {
    ...
    const legacyPath = publicUrl.startsWith(`${LEGACY_BRANDING_PREFIX}/`)
        ? path.join(REPO_ROOT, 'client', publicUrl.replace(/^\//, ''))
        : null;
